...
namespace WebCommon
{
    public class XapianLogic
    {
        ...
        /// <param name="filter">来源/等级过滤</param>
        ...
        /// <param name="filter">来源/等级过滤</param>
        /// <param name="filetype">类型过滤</param>
        /// <param name="xm">返回结果</param>
        public void SearchReturnforSilo(string query, int filter, string filetype, out Xapian.MSet xm)
        {
            query = query.Replace("\\", "");
            query = query.Replace("/", "");
            string querystr = _cs.JiebaSegnotSearch(query);     //分词
        ...
        #region nodecache
        static public void InitialCacheConfig(string path)
        {
            //判断是否存在文件
            if (File.Exists(path))
            {
                ...
                if (fi.Length == 0)
                {
                    ...
                    try
                    {
                        var xDoc = new XDocument(new XElement("savecache"));
        ...
        static public int InsertCacheNode(string path, string Name, int Count)
        {
            InitialCacheConfig(path);
            try
            {
                //定义并从xml文件中加载节点（根节点）
                ...
                //无数据插入
                XElement newNode = new XElement("savecache", new XAttribute("Name", Name),
        ...
        static public int ShowCacheXml(string path, out Dictionary<string, int> targetNodes)
        {
            targetNodes = null;
            try
            {
                ...
                //查询语句: 获得根节点下Net子节点
                IEnumerable<XElement> Nodes = from target in rootNode.Descendants("savecache")
                                              select target;

                foreach (var item in Nodes)
                {
                    //取值并重置
                    string Count = item.Element("Count").ToString();
                    Count = Count.Replace("</Count>", "");
                    Count = Count.Replace("<Count>", "");
                    targetNodes = new Dictionary<string, int>();
                    string Name = item.Attribute("Name").ToString();
                    Name = Name.Replace("\"", "");
                    Name = Name.Replace("Name=", "");
                    targetNodes.Add(Name, int.Parse(Count));
                }
            }
            catch (Exception e)
            {
        ...
        public void SearchforsecReturn(string query, int page, int filter, string filetype, out uint num, out List<SearchResult> XapResList)
        {
            Xapian.MSet xm;
            XapResList = new List<SearchResult>();
